...
class ConditionalModel(nn.Module):
    def __init__(self, n_steps, y_dim=10, fp_dim=128, feature_dim=4096, guidance=True, encoder_type='resnet34'):
        ...
        if encoder_type == 'resnet34':
            self.encoder_x = resnet_s.resnet34(num_input_channels=3, num_classes=feature_dim)
        elif encoder_type == 'resnet18':
            self.encoder_x = resnet_s.resnet18(num_input_channels=3, num_classes=feature_dim)
        elif encoder_type == 'resnet50':
            self.encoder_x = resnet_s.resnet50(num_input_channels=3, num_classes=feature_dim)
            ...
            self.encoder_x = resnet_l.resnet18(num_classes=feature_dim, pretrained=True)
        elif encoder_type == 'resnet34_l':
            self.encoder_x = resnet_l.resnet34(num_classes=feature_dim, pretrained=True)
        elif encoder_type == 'resnet50_l':
            self.encoder_x = resnet_l.resnet50(num_classes=feature_dim, pretrained=True)
        else:
...
class Diffusion(nn.Module):
    def __init__(self, fp_encoder, num_timesteps=1000, n_class=10, fp_dim=512, device='cuda', beta_schedule='linear',
        ...
        self.fp_encoder = fp_encoder.to(device)
