...
from utils.doc_data_utils import RVL_CDIP
import torch
...
def train(diffusion_model, train_loader, val_loader, test_loader, model_save_dir, n_epochs=1000, knn=10, data_dir='./Clothing1M_data'):
    ...
    test_embed = np.load(os.path.join(data_dir, 'fp_embed_test_doc.npy'))
    val_embed = np.load(os.path.join(data_dir, 'fp_embed_val_doc.npy'))
    train_embed_all = torch.tensor(np.load(os.path.join(data_dir, 'fp_embed_train_doc.npy'))).to(torch.float32)
    neighbours = np.load(os.path.join(data_dir, 'fp_knn_doc.npy'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fp_encoder", default='CLIP', help="encoder", type=str)
    ...
    # parser.add_argument("--data_dir", default='', help="dir to Clothing1M", type=str)
    parser.add_argument("--diff_encoder", default='resnet50_l', help="which encoder for diffusion", type=str)
    args = parser.parse_args()

    data_dir = os.path.join(os.getcwd(), 'rvl-cdip')
    ...
    train_dataset = RVL_CDIP(data_root=data_dir, split='train', cls_size=10000)
    labels = torch.tensor(train_dataset.targets)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                                              num_workers=num_workers,
                                              worker_init_fn=init_fn, drop_last=True)
    val_dataset = RVL_CDIP(data_root=data_dir, split='val', l_max=5000)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    test_dataset = RVL_CDIP(data_root=data_dir, split='test', l_max=10000)
    ...
    model_path = './model/RVL-CDIP_LRA-diffusion.pt'
    ...
    train_embed_dir = os.path.join(data_dir, 'fp_embed_train_doc.npy')
    ...
    val_embed_dir = os.path.join(data_dir, 'fp_embed_val_doc.npy')
    ...
    test_embed_dir = os.path.join(data_dir, 'fp_embed_test_doc.npy')
    ...
    neighbours = prepare_knn(labels, train_embed, os.path.join(data_dir, 'fp_knn_doc.npy'), k=args.k)
